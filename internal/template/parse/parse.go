...
func (p *parser) parseCurrentToken() {
	// PRESS PATCH: handle template tokens.
	// Classify the token and create the appropriate node type.
	if p.tok.Type == TemplateToken {
		kind, rest := ClassifyTemplate(p.tok.Data)
		switch kind {
		case ExprToken:
			expr, err := ParseExpr(rest)
			if err != nil {
				panic(fmt.Sprintf("invalid expression {%s}: %s", rest, err))
			}
			p.addChild(&Node{
				Type: ExpressionNode,
				Data: rest,
		...
		case IfOpenToken:
			expr, err := ParseExpr(rest)
			if err != nil {
				panic(fmt.Sprintf("invalid if condition {%s}: %s", rest, err))
			}
			p.addChild(&Node{
				Type: IfNode,
				Data: rest,
		...
		case EachOpenToken:
			listStr, binding, indexBinding := ParseEachBinding(rest)
			if listStr == "" || binding == "" {
				panic(fmt.Sprintf("invalid each: expected {each list as item}, got {each %s}", rest))
			}
			expr, err := ParseExpr(listStr)
			if err != nil {
				panic(fmt.Sprintf("invalid each expression {%s}: %s", listStr, err))
			}
			p.addChild(&Node{
				Type: EachNode,
				Data: rest,
		...
		case SnippetOpenToken:
			name, params := ParseSnippetDecl(rest)
			p.addChild(&Node{
				Type: SnippetNode,
				Data: name,
				TemplateData: &TemplateData{
		...
		case SnippetCloseToken:
			p.addChild(&Node{
				Type: snippetCloseNode,
			})
		case ConstToken:
			name, exprStr := ParseConstDecl(rest)
			var expr Expr
			if exprStr != "" {
				var err error
				expr, err = ParseExpr(exprStr)
				if err != nil {
					panic(fmt.Sprintf("invalid const expression {%s}: %s", exprStr, err))
				}
			}
			p.addChild(&Node{
				Type: ConstNode,
	...
	if p.tok.Type == SelfClosingTagToken {
		// PRESS PATCH: respect self-closing on vocabulary tags.
		// Treat them like void elements: add to tree, pop immediately.
		if isVocabularyTag(p.tok.Data) {
			p.tok.Type = StartTagToken
			p.reconstructActiveFormattingElements()
			p.addElement()
			p.oe.pop()
